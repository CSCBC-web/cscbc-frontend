...
export async function getFilteredSermonsMeta(page:number, speakers: string[], tagIds: string[]) {
...
export const getLocalizedTitle = (lng: string, sermon: SermonMetaType) => {
    if (lng === 'zh-Hant') return sermon.title_zhHant;
    if (lng === 'zh') return sermon.title_zh;
    return sermon.title_en;
}

export const getLocalizedTagNames = ( lng: string, tag: SermonCatType ) => {
    if (lng === 'zh-Hant') return tag.title_zhHant;
    if (lng === 'zh') return tag.title_zh;
    return tag.title_en;
}